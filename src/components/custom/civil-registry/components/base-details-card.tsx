...
import {
  AttachmentsTable,
  AttachmentWithCertifiedCopies,
} from "@/components/custom/civil-registry/components/attachment-table";
...
const createAttachment = (
  fileUrl: string,
  fileName: string
): Attachment & { certifiedCopies: never[] } => {
  return {
    id: "",
    userId: null,
    documentId: null,
...
export const BaseDetailsCard: React.FC<BaseDetailsCardProps> = ({
  form,
  onUpdateAction,
}) => {
  ...
  const [currentForm, setCurrentForm] =
    useState<BaseRegistryFormWithRelations>(form);
  ...
  const handleUploadSuccess = useCallback(
    (fileData: {
      ...
      const attachmentType =
        formData.formType === "MARRIAGE"
          ? "MARRIAGE_CERTIFICATE"
          : formData.formType === "BIRTH"
          ? "BIRTH_CERTIFICATE"
          : formData.formType === "DEATH"
          ? "DEATH_CERTIFICATE"
      ...
      {canEdit && (
        <EditCivilRegistryFormDialog
          ...
          onSaveAction={async (updatedForm) => {
            toast.success(
              `${t("Form updated successfully")} ${updatedForm.id}!`
            );
            setCurrentForm(updatedForm);
            onUpdateAction?.(updatedForm);
