...
const DelayedRegistrationForm: React.FC = () => {
  ...
  const [AffofficerAddressNcrMode, setOfficerAffiantAddressNcrMode] =
    useState(false);
  ...
  const province = watch(
    "affidavitOfDelayedRegistration.adminOfficer.address.province"
  );
  const province1 = watch(
  ...
  useEffect(() => {
    if (isDelayedRegistration) {
      ...
      const shouldBeNCR1 =
        provinceString1.trim().toLowerCase() === "metro manila";
      ...
      setValue(
        "affidavitOfDelayedRegistration.affiant.address.province",
        shouldBeNCR1 ? "Metro Manila" : provinceString1,
        {
          shouldValidate: true, // Trigger validation immediately
          shouldDirty: true, // Mark the field as dirty to ensure validation
        }
