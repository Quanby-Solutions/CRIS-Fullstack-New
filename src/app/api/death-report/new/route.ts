...
// Define a type for place of death categories
type PlaceOfDeathCategory = 'hospital' | 'barangay' | 'compound';

// Define a type for disposal method categories
type DisposalCategory = 'burial' | 'cremation';

// Define a type for transfer permit categories
type TransferPermitCategory = 'withTransferPermit' | 'withoutTransferPermit';
...
export async function GET(request: NextRequest) {
    try {
        ...
        const statistics: Statistics = {
            ...
            totalDocRegistered: deathRecords.length,
        ...
        deathRecords.forEach((record, index) => {
            if (record.registeredByDate) {
                ...
                // Increment total documents registered for the month
                statistics.monthly[monthKey].totalDocRegistered++;
                ...
                if (record.deathCertificateForm) {
                    ...
                    const isLateRegistration = getIsLateRegistration(form);
                    ...
                    // Process age groups
                    const ageYears = getAgeYears(form);
                    ...
                    // Process place of death
                    const placeOfDeathCategory = getPlaceOfDeathCategory(form);
                    recordInfo.placeOfDeath = placeOfDeathCategory;

                    statistics.placeOfDeath[placeOfDeathCategory]++;
                    statistics.monthly[monthKey].placeOfDeath[placeOfDeathCategory]++;

                    // Process disposal method
                    const disposalCategory = getDisposalCategory(form);
                    recordInfo.disposal = disposalCategory;

                    statistics.disposal[disposalCategory]++;
                    statistics.monthly[monthKey].disposal[disposalCategory]++;

                    // Process transfer permit
                    const transferPermitCategory = getTransferPermitCategory(form);
                    recordInfo.transferPermit = transferPermitCategory;

                    statistics.transferPermit[transferPermitCategory]++;
                    statistics.monthly[monthKey].transferPermit[transferPermitCategory]++;

                } else {
                    recordInfo.isLate = false;
                    recordInfo.gender = 'UNKNOWN';
                    recordInfo.ageGroup = 'unknown';
                    recordInfo.placeOfDeath = 'compound';
                    recordInfo.disposal = 'burial';
                    recordInfo.transferPermit = 'withoutTransferPermit';

                    statistics.gender.unknown++;
                    statistics.ageGroups.unknown++;
                    statistics.registration.onTime++;
                    statistics.placeOfDeath.compound++;
                    statistics.disposal.burial++;
                    statistics.transferPermit.withoutTransferPermit++;

                    statistics.monthly[monthKey].gender.unknown++;
                    statistics.monthly[monthKey].ageGroups.unknown++;
                    statistics.monthly[monthKey].registration.onTime++;
                    statistics.monthly[monthKey].placeOfDeath.compound++;
                    statistics.monthly[monthKey].disposal.burial++;
                    statistics.monthly[monthKey].transferPermit.withoutTransferPermit++;
                }
        ...
        const debug = {
            ...
            monthlyStats: Object.keys(statistics.monthly).map(month => {
                ...
                return {
                    ...
                    placeOfDeath: monthData.placeOfDeath,
                    disposal: monthData.disposal,
                    transferPermit: monthData.transferPermit,
                    totalDocRegistered: monthData.totalDocRegistered,
...
// Helper function to determine place of death category
function getPlaceOfDeathCategory(deathCertificateForm: any): PlaceOfDeathCategory {
    try {
        const placeOfDeath = deathCertificateForm.placeOfDeath as any;

        if (!placeOfDeath) return 'compound';

        const locationType = String(placeOfDeath.locationType || '').toLowerCase();
        const barangay = String(placeOfDeath.barangay || '').trim();

        // Check if location type is hospital
        if (locationType.includes('hospital')) {
            return 'hospital';
        }

        // Check if barangay has value and location type is not hospital
        if (barangay && barangay !== '' && !locationType.includes('hospital')) {
            return 'barangay';
        }
        ...
        console.error('Error determining place of death category:', error);
        return 'compound';
    }
}

// Helper function to determine disposal category
function getDisposalCategory(deathCertificateForm: any): DisposalCategory {
    try {
        const corpseDisposal = String(deathCertificateForm.corpseDisposal || '').toLowerCase();

        if (corpseDisposal.includes('cremation')) {
            return 'cremation';
        }
        ...
        console.error('Error determining disposal category:', error);
        return 'burial';
    }
}

// Helper function to determine transfer permit category
function getTransferPermitCategory(deathCertificateForm: any): TransferPermitCategory {
    try {
        const hasPermit = typeof deathCertificateForm.transferPermit === 'object' &&
            deathCertificateForm.transferPermit !== null &&
            'number' in deathCertificateForm.transferPermit;

        return hasPermit ? 'withTransferPermit' : 'withoutTransferPermit';

    } catch (error) {
        console.error('Error determining transfer permit category:', error);
        return 'withoutTransferPermit';
    }
}
