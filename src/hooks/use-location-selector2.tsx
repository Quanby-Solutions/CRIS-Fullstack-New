...
export const useLocationSelector2 = ({
  ...
  const [selectedProvince, setSelectedProvince] =
    useState<string>(defaultProvinceId);
  ...
  const findAndSetMunicipality = useCallback(
    (municipalities: LocationSuggestion[], municipalityValue: string) => {
      ...
      if (!matchedMunicipality) {
        matchedMunicipality = municipalities.find((m) =>
          m.displayName.toLowerCase().includes(municipalityValue.toLowerCase())
        );
      }
  ...
  const handleBarangayChange = async (value: string) => {
    ...
    // Store the current value in component state
